        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            <h4 class="card-title text-primary">Kelola Owner</h4>
                        </div>
                        <button class="btn btn-success" data-target="#tambahOwnerModal" data-toggle="modal"><i
    ...
    <div class="modal fade" id="tambahOwnerModal" tabindex="-1" role="dialog" aria-labelledby="tambahOwnerModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.kelola.owner.store') }}" method="POST" class="form-with-loading">
...
@push('js')
    ...
    <script>
        $(document).ready(() => {
            $('#table-1').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.kelola.owner.data') }}",
                    data: function(e) {
                        return e;
                    }
                },
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            const deleteUrl =
                                "{{ route('admin.kelola.owner.delete', ':id') }}"
    ...
    <script>
        const edit = (id) => {
            $.getJSON(`${window.location.origin}/admin/kelola/owner/data/${id}`, (data) => {
                const editUrl =
                    `{{ route('admin.kelola.owner.store', ':id') }}`

                $('#formEditOwner').attr('action', editUrl.replace(':id', id))
        ...
        const changeJustPassword = (id) => {
            const changeUrl = "{{ route('change.just.password', ':id') }}"

            $('#formChangePassword').attr('action', changeUrl.replace(':id', id))
