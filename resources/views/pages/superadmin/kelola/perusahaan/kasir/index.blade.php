    ...
    <div class="modal fade" id="tambahKasirModal" tabindex="-1" role="dialog" aria-labelledby="tambahKasirModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('superadmin.kelola.perusahaan.kasir.store', ['perusahaanId' => $perusahaan->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form id="formEditKasir"
                    action="{{ route('superadmin.kelola.perusahaan.kasir.store', ['perusahaanId' => $perusahaan->id]) }}"
                    method="POST" class="form-with-loading">
                    <div class="modal-body">
                        @csrf
                        <div class="form-group">
...
@push('js')
    ...
    <script>
        $(document).ready(() => {
            $('#table-1').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('superadmin.kelola.perusahaan.kasir.data', ['perusahaanId' => $perusahaan->id]) }}",
                    data: function(e) {
                        return e;
                    }
                },
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            const deleteUrl =
                                "{{ route('superadmin.kelola.perusahaan.kasir.data.delete', ['perusahaanId' => ':perusahaanId', 'kasirId' => ':kasirId']) }}"
                            ...
                            let deleteBtn =
                                `<a href='${deleteUrl.replace(":perusahaanId", perusahaanId).replace(":kasirId", row.id)}' class="btn btn-danger" data-confirm-delete="true"><i class="fa-regular fa-trash"></i></a>`;
    ...
    <script>
        const edit = (kasirId) => {
            const perusahaanId = '{{ $perusahaan->id }}'
            $.getJSON(`${window.location.origin}/superadmin/kelola/perusahaan/${perusahaanId}/kasir/data/${kasirId}`, (
                data) => {
                const editUrl =
                    `{{ route('superadmin.kelola.perusahaan.kasir.store', ['perusahaanId' => ':perusahaanId', 'kasirId' => ':kasirId']) }}`

                $('#formEditKasir').attr('action', editUrl.replace(':perusahaanId', perusahaanId).replace(
                    ':kasirId', kasirId))
