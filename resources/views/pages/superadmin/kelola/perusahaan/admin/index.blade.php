@extends('layouts.app')
@section('title', 'Kelola Admin - ' . $perusahaan->nama)
...
@section('content')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col">
                            <h4 class="card-title text-primary">Kelola Admin | {{ $perusahaan->nama }}</h4>
                        </div>
                        <button class="btn btn-success" data-target="#tambahAdminModal" data-toggle="modal">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="tambahAdminModalLabel">Tambah Admin | {{ $perusahaan->nama }}</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form action="{{ route('superadmin.kelola.perusahaan.admin.store', ['perusahaanId' => $perusahaan->id]) }}"
    ...
    <div class="modal fade" id="editAdminModal" tabindex="-1" role="dialog" aria-labelledby="editAdminModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editAdminModalLabel">Edit Admin | {{ $perusahaan->nama }}</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="changePasswordModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="changePasswordModalLabel">Change Password Admin | {{ $perusahaan->nama }}
                    </h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
...
@push('js')
    ...
    <script>
        $(document).ready(() => {
            $('#table-1').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('superadmin.kelola.perusahaan.admin.data', ['perusahaanId' => $perusahaan->id]) }}",
                    data: function(e) {
                        return e;
                    }
                },
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            let deleteBtn =
                                `<a href='${deleteUrl.replace(":perusahaanId", perusahaanId).replace(":adminId", row.id)}' class="btn btn-danger" data-confirm-delete="true"><i class="fa-regular fa-trash"></i></a>`;
    ...
    <script>
        const edit = (adminId) => {
            const perusahaanId = '{{ $perusahaan->id }}'
            $.getJSON(`${window.location.origin}/superadmin/kelola/perusahaan/${perusahaanId}/admin/data/${adminId}`, (
                data) => {
                const editUrl =
                    `{{ route('superadmin.kelola.perusahaan.admin.store', ['perusahaanId' => ':perusahaanId', 'adminId' => ':adminId']) }}`

                $('#formEditAdmin').attr('action', editUrl.replace(':perusahaanId', perusahaanId).replace(
                    ':adminId', adminId))
                $('#editNamaLengkap').val(data.user.nama_lengkap)
                $('#editUsername').val(data.user.username)
                $('#editNomorTelp').val(data.user.nomor_telp)

                const myModal = new bootstrap.Modal(document.getElementById('editAdminModal'))
                myModal.show()
            })
        }
